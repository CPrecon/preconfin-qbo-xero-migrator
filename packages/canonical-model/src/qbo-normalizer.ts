...
function status(value: unknown): TransactionStatus {
  ...
  if (["draft", "authorized", "paid", "void", "deleted"].includes(raw))
...
function linkedTransactionId(txn: any): string {
  ...
  if (type === "bill") return compactId("bill", txn.TxnId);
  if (type === "creditmemo") return compactId("credit", txn.TxnId);
  if (type === "vendorcredit") return compactId("supplier_credit", txn.TxnId);
...
export function normalizeQboDataset(raw: QboRawDataset): AccountingSnapshot {
  ...
  const baseCurrency =
    firstString(
      company.Country === "CA" ? "CAD" : undefined,
      company.CurrencyRef?.value,
      "USD",
    ) ?? "USD";
  const organization = {
  ...
  const accounts: Account[] = raw.accounts.map((account) => ({
    ...
    name:
      firstString(account.Name, account.FullyQualifiedName, account.Id) ??
      String(account.Id),
    ...
    parentId: account.ParentRef?.value
      ? compactId("acct", account.ParentRef.value)
      : undefined,
    currentBalance: money(
      account.CurrentBalance ?? 0,
      firstString(account.CurrencyRef?.value, baseCurrency),
    ),
  }));
  ...
  const items: Item[] = raw.items.map((item) => ({
    ...
    unitPrice:
      item.UnitPrice === undefined
        ? undefined
        : money(item.UnitPrice, baseCurrency),
    purchasePrice:
      item.PurchaseCost === undefined
        ? undefined
        : money(item.PurchaseCost, baseCurrency),
    incomeAccountId: item.IncomeAccountRef?.value
      ? compactId("acct", item.IncomeAccountRef.value)
      : undefined,
    expenseAccountId: item.ExpenseAccountRef?.value
      ? compactId("acct", item.ExpenseAccountRef.value)
      : undefined,
    inventoryAssetAccountId: item.AssetAccountRef?.value
      ? compactId("acct", item.AssetAccountRef.value)
      : undefined,
  ...
  const invoices: Invoice[] = raw.invoices.map((invoice) => {
    ...
    return {
      ...
      contactId: invoice.CustomerRef?.value
        ? compactId("contact_customer", invoice.CustomerRef.value)
        : undefined,
  ...
  const bills: Bill[] = raw.bills.map((bill) => {
    ...
    return {
      ...
      contactId: bill.VendorRef?.value
        ? compactId("contact_supplier", bill.VendorRef.value)
        : undefined,
  ...
  const payments: Payment[] = raw.payments.map((payment) => ({
    ...
    contactId: payment.CustomerRef?.value
      ? compactId("contact_customer", payment.CustomerRef.value)
      : undefined,
    date: normalizeDate(payment.TxnDate),
    accountId: payment.DepositToAccountRef?.value
      ? compactId("acct", payment.DepositToAccountRef.value)
      : undefined,
    amount: money(
      payment.TotalAmt ?? payment.UnappliedAmt ?? 0,
      firstString(payment.CurrencyRef?.value, baseCurrency),
    ),
    appliedTo: (payment.Line ?? []).flatMap((line: any) =>
      (line.LinkedTxn ?? []).map((txn: any) => ({
        transactionId: linkedTransactionId(txn),
  ...
  const credits: Credit[] = [
    ...raw.creditMemos.map((credit) => ({
      ...
      contactId: credit.CustomerRef?.value
        ? compactId("contact_customer", credit.CustomerRef.value)
        : undefined,
    ...
    ...raw.vendorCredits.map((credit) => ({
      ...
      source: sourceRef(credit.Id, "credit", credit),
      number: firstString(credit.DocNumber, credit.Id),
      contactId: credit.VendorRef?.value
        ? compactId("contact_supplier", credit.VendorRef.value)
        : undefined,
      date: normalizeDate(credit.TxnDate),
  ...
  const currencies: Currency[] = raw.currencies.length
    ? raw.currencies.map((currency) => ({
        ...
        code:
          firstString(currency.Code, currency.Name, baseCurrency) ??
          baseCurrency,
    ...
    : [
        {
          id: compactId("currency", baseCurrency),
          source: sourceRef(baseCurrency, "currency", { base: true }),
          code: baseCurrency,
          active: true,
        },
      ];
  ...
  const tracking: TrackingCategory[] = [
    ...raw.classes.map((item) => ({
      ...
      name: "Class",
      option:
        firstString(item.Name, item.FullyQualifiedName, item.Id) ??
        String(item.Id),
      active: item.Active !== false,
    })),
