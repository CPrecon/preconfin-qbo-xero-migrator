...
function keyValue(doc: PDFKit.PDFDocument, key: string, value: string | number): void {
  doc.fontSize(10).fillColor("#5a6673").text(key, { continued: true }).fillColor("#16202a").text(`  ${value}`);
}

export async function generateMigrationHealthPdf({ snapshot, plan, validation }: PdfReportInput): Promise<Buffer> {
  const doc = new PDFDocument({ size: "LETTER", margin: 56, info: { Title: "PreconFin Migration Health Report", Author: "PreconFin" } });
  const done = collect(doc);

  doc.rect(0, 0, 612, 96).fill("#16202a");
  doc.fillColor("#ffffff").fontSize(23).text("PreconFin Migration Health Report", 56, 34);
  doc.fontSize(10).fillColor("#c9d7d3").text("QBO to Xero migration readiness", 56, 64);

  doc.fillColor("#16202a").fontSize(24).text(snapshot.organization.displayName, 56, 130);
  doc.fontSize(11).fillColor("#5a6673").text(`Generated ${new Date(validation.summary.generatedAt).toLocaleString("en-US")}`);
  ...
  keyValue(doc, "QuickBooks realm", snapshot.organization.qboRealmId ?? "Not provided");

  section(doc, "Readiness");
  const readinessText = validation.summary.readiness === "ready"
  ...
  for (const finding of findings) {
    doc.fontSize(10).fillColor(finding.severity === "error" ? "#a32929" : "#946200").text(`${finding.severity.toUpperCase()} — ${finding.title}`);
    doc.fillColor("#16202a").text(finding.message, { indent: 12, lineGap: 2 });
    doc.fillColor("#5a6673").text(finding.recommendation, { indent: 12, lineGap: 4 });
  }

  section(doc, "Account Mapping");
  for (const mapping of plan.accountMappings.slice(0, 18)) {
    doc.fontSize(9).fillColor("#16202a").text(`${mapping.sourceName} → ${mapping.targetCode ?? ""} ${mapping.targetName} (${mapping.targetType})`);
  }

  section(doc, "Recommendations");
  for (const recommendation of validation.recommendations) {
    doc.fontSize(10).fillColor("#16202a").text(`• ${recommendation}`, { lineGap: 4 });
  }

  section(doc, "Next Steps");
  doc.fontSize(10).fillColor("#16202a").text("1. Resolve errors and review warnings.", { lineGap: 4 });
  doc.text("2. Import CSV files into a Xero demo organization first.", { lineGap: 4 });
  doc.text("3. Reconcile AR, AP, bank balances, retained earnings, and tax balances.", { lineGap: 4 });
  doc.text("4. Book a PreconFin consultation for assisted migration review.", { lineGap: 4 });
