...
import { randomToken, signState, verifySignedState } from "./security/tokens.js";
...
export async function buildServer(env: AppEnv) {
  ...
  await app.register(cors, { origin: env.CORS_ORIGINS.split(",").map((item) => item.trim()), credentials: false });
  ...
  app.get("/health", async () => ({ ok: true, service: "qbo-xero-migrator-api", timestamp: new Date().toISOString() }));
  ...
  app.get("/api/oauth/qbo/start", async (request, reply) => {
    ...
    return reply.redirect(oauth.authorizationUrl(state));
  });

  app.get("/api/oauth/qbo/callback", async (request, reply) => {
    const query = z.object({ code: z.string(), state: z.string(), realmId: z.string() }).parse(request.query);
    const nonce = verifySignedState(query.state, env.OAUTH_STATE_SIGNING_SECRET);
    ...
    const tokens = await oauth.exchangeCode(query.code, query.realmId);
    const encryptedTokens = encryptJson(tokens, env.TOKEN_ENCRYPTION_KEY);
    const { connection, token } = await repo.createConnection({ realmId: query.realmId, encryptedTokens });
    await repo.audit("qbo_connection_created", { connectionId: connection.id, realmId: query.realmId });
  ...
  app.post("/api/oauth/qbo/disconnect", async (request, reply) => {
    const body = z.object({ connectionId: z.string().uuid(), connectionToken: z.string().min(16) }).parse(request.body);
    await repo.deleteConnection(body.connectionId, body.connectionToken);
    await repo.audit("qbo_connection_deleted", { connectionId: body.connectionId });
    return reply.send({ ok: true });
  });

  app.post("/api/migration-jobs", async (request, reply) => {
    const body = z.object({ connectionId: z.string().uuid(), connectionToken: z.string().min(16) }).parse(request.body);
    const connection = await repo.getConnection(body.connectionId, body.connectionToken);
    if (!connection) return reply.code(404).send({ error: "QuickBooks connection not found" });
    const { job, token } = await repo.createJob(connection.id);
    await repo.audit("migration_job_created", { jobId: job.id, connectionId: connection.id });
    return reply.code(201).send({ jobId: job.id, jobToken: token, status: job.status });
  });

  app.post("/api/migration-jobs/:id/run", async (request, reply) => {
    const params = z.object({ id: z.string().uuid() }).parse(request.params);
    const token = tokenFromRequest(request);
    if (!token) return reply.code(401).send({ error: "Migration token required" });
  ...
  app.get("/api/migration-jobs/:id", async (request, reply) => {
    ...
    return reply.send({ id: job.id, status: job.status, readinessScore: job.readinessScore, readinessStatus: job.readinessStatus, errorMessage: job.errorMessage, createdAt: job.createdAt, updatedAt: job.updatedAt });
  });
  ...
  app.get("/api/migration-jobs/:id/downloads", async (request, reply) => {
    ...
    const downloads = await Promise.all(artifacts.map(async (artifact) => ({
      id: artifact.id,
      kind: artifact.kind,
      contentType: artifact.contentType,
      sizeBytes: artifact.sizeBytes,
      expiresAt: artifact.expiresAt,
      url: await repo.signedArtifactUrl(env.SUPABASE_STORAGE_BUCKET, artifact.path)
    })));
  ...
  app.delete("/api/migration-jobs/:id", async (request, reply) => {
    ...
    const token = tokenFromRequest(request);
    if (!token) return reply.code(401).send({ error: "Migration token required" });
    await repo.deleteJob(params.id, token);
    await repo.audit("migration_job_deleted", { jobId: params.id });
    return reply.send({ ok: true });
  });
  ...
  app.post("/api/leads", async (request, reply) => {
    ...
    await repo.audit("lead_captured", { email: body.email, jobId: body.jobId, source: body.source });
